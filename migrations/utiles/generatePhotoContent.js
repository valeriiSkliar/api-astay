...
async function generateContent(tableName) {
  ...
  let imageCounter = 1;
  const apartmentPhotoDataArray = allApartments.map(apartment => {
    const photosData = [];
    for (let i = 0; i < 10; i++) {
      photosData.push({
  ...
  const complexPhotoDataArray = allComplexes.map(complex => {
    const photosData = [];
    for (let i = 0; i < 10; i++) {
      photosData.push({
        url: generateImageUrl(imageCounter),
  ...
  const photoDataInstansesArray = apartmentPhotoDataArray
