...
export class UploaderService {
  ...
  public async repositoryFabric(values: {
    apartment_id?: number;
    complex_id?: number;
  }) {
  ...
  public async reBuildOrderIndexesWhenDeletingPhoto({
    apartment_id,
    complex_id,
  }: {
    apartment_id?: number;
    ...
    if (apartment_id) {
      const images =
        await this.reBuildOrderIndexesWhenDeletingPhotoByApartmentId(
          apartment_id,
        );
      return images;
    } else if (complex_id) {
      const images =
        await this.reBuildOrderIndexesWhenDeletingPhotoByComplexId(complex_id);
      return images;
    }
  }
  private async reBuildOrderIndexesWhenDeletingPhotoByComplexId(
    complex_id: number,
  ) {
    ...
    if (complexImages.length > 0) {
      for (let index = 0; index < complexImages.length; index++) {
        ...
        if (image.order_number !== newOrderNumber) {
          await this.photoRepository.updateById(image.id, {
  ...
  private async reBuildOrderIndexesWhenDeletingPhotoByApartmentId(
    apartment_id: number,
  ) {
