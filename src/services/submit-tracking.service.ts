...
export class SubmitTrackingService {
  ...
  public incrementSubmissionCount(clientIp: string): void {
    ...
    const cachedSubmissionCount = SubmitTrackingService.statucIpCache.get(clientIp);
    if (cachedSubmissionCount === undefined) {
      SubmitTrackingService.statucIpCache.set(clientIp, 1);
    } else {
      SubmitTrackingService.statucIpCache.set(clientIp, cachedSubmissionCount + 1);
    }
...
export const SUBMIT_TRACKING_SERVICE = BindingKey.create<SubmitTrackingService>('services.submit-tracking');
